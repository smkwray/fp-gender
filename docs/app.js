...
function renderHeroSubtitle() {
  if (!dom.pageSubtitle || !state.manifest) {
    return;
  }
  const heroText = `${state.manifest.hero_text || "Gender-focused Fair model runs exported from fp-wraptr."}`.trim();
  const topLinks = Array.isArray(state.manifest.top_links) ? state.manifest.top_links : [];
  dom.pageSubtitle.innerHTML = "";
  dom.pageSubtitle.append(document.createTextNode(heroText));
  for (const link of topLinks) {
    ...
    dom.pageSubtitle.append(document.createTextNode(" "));
    const anchor = document.createElement("a");
    anchor.className = "hero-inline-link";
    anchor.href = url;
    anchor.textContent = label;
    dom.pageSubtitle.append(anchor);
  }
}
...
async function initialize() {
  ...
  dom.pageTitle.textContent = manifest.title || "FP Gender Runs Explorer";
  renderHeroSubtitle();
